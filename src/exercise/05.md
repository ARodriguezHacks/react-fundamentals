...
You can use the spread operator ` ...example` in order to 'forward' props to for instance a child component.

This example below is taken from the official React docs, where we can see the the parent component, Profile, accepting props, and then passing them or forwarding all of the props to a child component, Avatar using the spread syntax. Using the spread operator reduces the need to type out all the props desired to be passed along to nested components, and if there are existing props,then the spread operator can be used to add onto, or overwrite the existing props depending on the order of the props being passed to the component.
...
function Profile(props) {
  return (
    <div className="card">
      <Avatar {...props} />
      // <Avatar name="Angie" {...props} />
      // Above example shows additional 'name' prop along with props passed from parent component.
      // <Avatar {...props} name="Angie" />
      // Same as above but with the order flipped around. Depending on the props being passed and being spread out, this could have unindended consequences, therefore the order matters.
    </div>
  );
}
```
